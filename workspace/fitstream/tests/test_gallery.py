...
class TestGalleryManager:
    ...
    def test_stats(self, gallery):
        gallery.add_from_job(job_id="x1", video_path="/v.mp4", type="animate",
                             duration_seconds=3.0, generation_time=40.0)
        gallery.add_from_job(job_id="x2", video_path="/v.mp4", type="story",
                             duration_seconds=10.0, generation_time=120.0)
...
class TestGalleryPersistence:
    def test_persist_and_reload(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            g1.add_from_job(job_id="persist1", video_path="/v.mp4",
                           prompt="Persistent item", style="anime")
...
class TestGalleryItem:
    def test_matches_search(self):
        item = GalleryItem(
            id="t1", video_path="/v.mp4",
            prompt="sunset over the ocean", style="cinematic",
            tags=["romantic", "beach"],
        )

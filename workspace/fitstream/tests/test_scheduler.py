...
from fitstream.core.scheduler import Scheduler, ScheduledJob


class TestScheduledJob:
    def test_one_shot(self):
        j = ScheduledJob(id="j1", name="test", job_type="animate",
                         params={}, run_at=time.time() - 10)
        assert j.is_due is True
        assert j.is_recurring is False
    
    def test_not_yet_due(self):
        j = ScheduledJob(id="j2", name="test", job_type="animate",
                         params={}, run_at=time.time() + 3600)
        assert j.is_due is False
    
    def test_inactive_not_due(self):
        j = ScheduledJob(id="j3", name="test", job_type="animate",
                         params={}, run_at=time.time() - 10, active=False)
        assert j.is_due is False
    
    def test_recurring(self):
        j = ScheduledJob(id="j4", name="test", job_type="animate",
                         params={}, run_at=time.time(), interval_seconds=3600)
        assert j.is_recurring is True
    
    def test_advance_oneshot(self):
        j = ScheduledJob(id="j5", name="test", job_type="animate",
                         params={}, run_at=time.time())
        j.advance()
        assert j.active is False
        assert j.runs_completed == 1
    
    def test_advance_recurring(self):
        j = ScheduledJob(id="j6", name="test", job_type="animate",
                         params={}, run_at=time.time(), interval_seconds=3600)
        old_run_at = j.run_at
        j.advance()
    ...
    def test_max_runs(self):
        j = ScheduledJob(id="j7", name="test", job_type="animate",
                         params={}, run_at=time.time(), interval_seconds=3600, max_runs=2)
        j.advance()
        assert j.active is True
        j.advance()
    ...
    def test_to_dict(self):
        j = ScheduledJob(id="j8", name="Test", job_type="animate",
                         params={}, run_at=time.time())
        d = j.to_dict()
        assert d["id"] == "j8"
        assert d["name"] == "Test"
...
class TestScheduler:
    ...
    def test_cancel(self):
        s = Scheduler()
        jid = s.schedule_once(datetime.now() + timedelta(hours=1),
                               "animate", {})

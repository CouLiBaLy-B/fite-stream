...
PIPELINE_FILES = [f for f in PIPELINE_DIR.glob("*.py") if f.name not in ("__init__.py", "base.py")]
...
class TestPipelineStructure:
    ...
    def test_imports_loguru(self, pipeline_file: Path) -> None:
        ...
        source = pipeline_file.read_text()
        assert (
            "from loguru import logger" in source
...
class TestCodeQualityMetrics:
    ...
    def test_no_circular_imports_in_pipelines(self) -> None:
        ...
        for f in PIPELINE_FILES:
            source = f.read_text()
            assert (
                "from fitstream.api" not in source

...
from fitstream.api.dependencies import (
    ...
    require_rate_limit, save_upload,
)
...
async def list_styles() -> dict:
    ...
    return {
        "styles": {k: {"label": v["label"], "description": v["suffix"][:80]}
...
async def fill_template(
    ...
    from fitstream.core.prompt_templates import PromptTemplateLibrary
    result = PromptTemplateLibrary().get(template_id, person=person, garment=garment, location=location)
    if result is None:
...
async def export_video(
    ...
    if format in format_map:
        ...
        result = exporter.to_social(video_path, f"{base}_{format.replace(':', 'x')}.mp4", aspect=format)
    else:
...
async def list_postprocess_presets() -> dict:
    ...
    return {
        ...
        "operations": ["upscale", "stabilize", "color_grade", "slow_motion", "loop", "add_watermark", "trim"],
    }
...
async def get_translations(lang: str) -> dict:
    ...
    i18n = I18n(lang)
    return {"language": i18n.lang, "supported": I18n.supported_languages(), "messages": i18n.get_all()}
...
async def register_webhook(url: str = Form(...), events: str = Form("completed,failed"), secret: str = Form("")) -> dict:
    from fitstream.core.webhooks import webhook_manager
    eid = webhook_manager.register(url, events=[e.strip() for e in events.split(",") if e.strip()], secret=secret)

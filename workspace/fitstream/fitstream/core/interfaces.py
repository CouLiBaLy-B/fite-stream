...
from typing import Protocol, Optional, List, Dict, Any, Iterator, runtime_checkable
...
def validate_image_upload(
    ...
    if ext not in ALLOWED_IMAGE_TYPES:
        errors.append(ValidationError(
            "file", f"Invalid image type: {ext}. Allowed: {ALLOWED_IMAGE_TYPES}",
            "invalid_type",
        ))
    ...
    if size_mb > MAX_UPLOAD_SIZE_MB:
        errors.append(ValidationError(
            "file", f"File too large: {size_mb:.1f}MB. Max: {MAX_UPLOAD_SIZE_MB}MB",
            "too_large",
        ))
    
    if file_size_bytes < 100:
        errors.append(ValidationError(
            "file", "File too small (likely empty or corrupt)",
            "too_small",
        ))
...
def validate_prompt(prompt: str, field_name: str = "prompt") -> List[ValidationError]:
    ...
    if not prompt or not prompt.strip():
        ...
        errors.append(ValidationError(
            field_name, f"Prompt too long: {len(prompt)} chars. Max: {MAX_PROMPT_LENGTH}",
            "too_long",
        ))
...
def validate_generation_params(
    ...
    if width < MIN_IMAGE_DIMENSION or width > MAX_IMAGE_DIMENSION:
        errors.append(ValidationError("width", f"Width must be {MIN_IMAGE_DIMENSION}-{MAX_IMAGE_DIMENSION}"))
    if height < MIN_IMAGE_DIMENSION or height > MAX_IMAGE_DIMENSION:
        errors.append(ValidationError("height", f"Height must be {MIN_IMAGE_DIMENSION}-{MAX_IMAGE_DIMENSION}"))

...
class AnalyticsEngine:
    ...
    def get_report(self, hours: float = 24) -> dict[str, Any]:
        ...
        p95_time = (
            gen_times_sorted[int(len(gen_times_sorted) * 0.95)] if len(gen_times_sorted) > 1 else 0
        )
        ...
        avg_time_by_type = {t: sum(times) / len(times) for t, times in time_by_type.items()}

...
from fitstream.core.utils.video_utils import concatenate_videos, get_video_info, save_video
    ...
    def extend(
        ...
        try:
            ...
            while current_duration < target_duration and num_extensions < max_extensions:
                logger.info(
                    f"   Extension {num_extensions + 1}: {current_duration:.1f}s → generating..."
                )
                ...
                chunk_path = os.path.join(
                    os.path.dirname(output_path), f"_extend_chunk_{num_extensions}.mp4"
                )
                ...
                if len(frames_to_save) > 0:
                    ...
                    logger.info(
                        f"   → Added {chunk_duration:.1f}s (total: {current_duration:.1f}s)"
                    )
                else:
                    break

...
def validate_image_references(prompt: str, num_images: int) -> list[str]:
    ...
    for i in range(1, num_images + 1):
        if i not in ref_indices:
            warnings.append(
                f"Image {i} provided but not referenced in prompt. Add @Image {i} to use it."
            )
...
class LoomPipeline(BasePipeline):
    ...
    def generate(  # type: ignore[override]
        ...
        if task not in self.SUPPORTED_TASKS:
            return LoomResult(
                ...
                task=task,
                success=False,
                error=f"Unknown task: {task}. Use: {self.SUPPORTED_TASKS}",
            )
        ...
        if output_path is None:
            ...
            output_path = os.path.join(
                self.config.output_dir, f"loom_{task}_{timestamp}_{seed}.mp4"
            )
        ...
        logger.info("🎨 LoomVideo Generation:")
    ...
    def _generate_loomvideo(
        ...
        cmd = [
            ...
            "--config_path",
            str(loom_repo / "configs" / "inference" / "generation.yaml"),
            "--ckpt_path",
            str(loom_dir),
            "--task",
            task,
            ...
            prompt,
            "--ref_image_paths",
            *[str(Path(img).resolve()) for img in images],
            "--height",
            str(height),
            "--width",
            str(width),
        ...
        logger.info("   Running LoomVideo inference...")
